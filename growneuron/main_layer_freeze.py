...
def main(argv):
  ...
  logging.get_absl_handler().use_absl_log_file('logs', FLAGS.output_dir) 
  ...
  logging.info('device %s', tf.config.get_visible_devices())
  ...
  logging.info('number of epochs %s', config.train_epochs)
  print("number of epochs", config.train_epochs)
  ...
  logging.info('printing dense layer weights')
  for i in range(len(model.layers)):
    name=getattr(model.layers[i], 'name')
    if (name=='dense'):
  ...
  latest_checkpoint = tf.train.latest_checkpoint(FLAGS.output_dir) #cherche dans le repertoire des checkpoint s'il y a un checkpoint a restorer
  ...
  if latest_checkpoint: #pour redemarrer depuis un checkpoint attention a leur to do je pense il manque des choses
      ...
      checkpoint.restore(latest_checkpoint) #on restore le checkpoint
      logging.info('Loaded checkpoint %s', latest_checkpoint)
      print('Loaded checkpoint', latest_checkpoint)
      initial_epoch = optimizer.iterations.numpy() // steps_per_epoch #pour trouver l'epoch depuis laquel on redemarre automatiquement
      logging.info("initial epoch %s", initial_epoch) 
  ...
  def train_step(iterator):
    ...
    for ind in tf.range(tf.cast(steps_per_epoch, tf.int32)):#parcours des batches/iteration de l'epoch
      ...
      inputs=next(iterator) #getting next batch
      images, labels = inputs
      ...
      if optimizer.iterations % config.get('log_freq', 100) == 1:
        ...
        print("train loss", train_metrics['train/loss'].result(), "train accuracy", train_metrics['train/accuracy'].result()*100)
  ...
  #computing number of "freezable" layer (conv2D ou linear, we don't freeze batch norm layers)
  nb_freezable_layers=0
  for l in model.layers:
    print(l.name)
    if ('conv' in l.name or 'dense' in l.name):
      nb_freezable_layers+=1

  logging.info("nb freezable layers (nb conv and dense layers) %s", nb_freezable_layers)
  print("nb freezable layers (nb conv and dense layers)", nb_freezable_layers)
  

  trainable_params = np.sum([np.prod(v.get_shape()) for v in model.trainable_weights])
  non_trainable_params = np.sum([np.prod(v.get_shape()) for v in model.non_trainable_weights])
  print("before freezing: nb trainable param", trainable_params, "nb non trainable params", non_trainable_params)
  logging.info("before freezing: nb trainable param %s, nb non trainable params %s", trainable_params, non_trainable_params)
  ...
  for epoch in range(initial_epoch, config.train_epochs): #parcours des epochs, boucle de training globale
    ...
    print("Starating layer freezing")
    #on commence par remettre tous les layers en trainable sinon on va finir par tout freezer
    for l in model.layers:
      if ('conv' in l.name or 'dense' in l.name):
        l.trainable=True

    freeze_probs=np.random.uniform(low=0, high=1, size=nb_freezable_layers)
    survival=np.random.binomial(n=1, p=freeze_probs) #n=1 because we need a bernouli distribution
    print(survival)
    logging.info("benoulli vector %s", survival)

    #going through layers and freezing layers with survival[indice]=0
    indice=0
    for l in model.layers:
      if ('conv' in l.name or 'dense' in l.name):
        if (survival[indice]==0):
          l.trainable=False
        indice+=1

    trainable_params = np.sum([np.prod(v.get_shape()) for v in model.trainable_weights])
    non_trainable_params = np.sum([np.prod(v.get_shape()) for v in model.non_trainable_weights])
    print("while freezing: nb trainable param", trainable_params, "nb non trainable params", non_trainable_params)
    logging.info("while freezing: nb trainable param %s, nb non trainable params %s", trainable_params, non_trainable_params)
    

    #on appelle la fonction train_step qui parcours les batch(donc boucle interne a l'epoch)
    
    nb_iters= train_step(train_iterator) #le train_iterator iter sur le train_set
    #on transmet les derniers gradients de l'iter precedente comme ca si le premier batch de la nouvelle epoch n'est pas selectionne on fera la maj sur
    #les dernier gradient de la precedente epoch, on doit faire ca sinon on a une erreur
    ...
    print("running eval of epoch", epoch)
  ...
  logging.info("nb total batches %s", nb_total_iters) 
