...
def main(argv):
  ...
  logging.info("pruning type %s pruning density %s", prune_type, density)
  ...
  mask_file=os.path.join(FLAGS.output_dir, "mask_copy.pkl")
  logging.info("mask file path %s", mask_file)
  mask=pruning.Masking()

  if (latest_checkpoint):
      f = open(mask_file, "rb")
      mask_cur=pickle.load(f)
      mask.add_module_without_compute_mask(model, density) #on cree juste des masks sans appliquer la methode d'elagage
      mask.masks=mask_cur #on charge les masks stockes
      logging.info("loaded masks %s", mask.masks)
      print("loaded masks ", mask.masks)
      f.close()
  else:
      mask.add_module(model, density=density, prune_type=prune_type)
      #on sauvgarde les masks pour pouvoir les charger par la suite si on fait un checkpoint dans le training
      f = open(mask_file, "wb")
      pickle.dump(mask.masks, f)
      f.close()
      logging.info("computed masks %s", mask.masks)
      print("computed masks ", mask.masks)

      

  """for i in range(len(model.layers)):
    name=getattr(model.layers[i], 'name')
    if name=='glayer_conv2d_26': #on exclue les layers de batch norm, pooling et flatten car on ne les elague pas  
  ...
  def train_step(iterator):
    ...
    for ind in tf.range(tf.cast(steps_per_epoch, tf.int32)):#parcours des batches/iteration de l'epoch
      ...
      mask.apply_mask() # we apply masks again to keep pruned weights at zero after optimizer update 

      print("checking if pruned weights are still at zero in iter", ind)
      total_size = 0
      sparse_size = 0
      for i in range(len(model.layers)):
            name=getattr(model.layers[i], 'name')
      ...
      if (ind==0):
        logging.info('Total parameters under sparsity level of %s: %s', density, sparse_size / total_size)
  ...
  total_size=0
  sparse_size=0
  print("checking that the pruned weights are still at zero")
  for i in range(len(model.layers)):
        name=getattr(model.layers[i], 'name')
        if name in mask.masks:
            weight=model.layers[i].weights[0]
            print(name, 'density:',np.sum((weight!=0))/tf.size(weight))
            total_size+=tf.size(weight)
            sparse_size+=np.sum((weight!=0))
