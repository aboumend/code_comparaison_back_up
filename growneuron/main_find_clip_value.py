...
def main(argv):
  ...
  grow_dataset = strategy.distribute_datasets_from_function( #si on fait un training distribue, la c'est pour le batch utilise pour le growing
      data.build_input_fn(ds_builder, grow_batch_size, topology=None,#ATTENTION: a garder meme si on fait pas du distribue, c'est la partie pretraitement du dataset
                          is_training=True,
                          cache_dataset=config.cache_dataset))
  logging.info("size of the batch used for growing %s",grow_batch_size)
  print("size of the batch used for growing", grow_batch_size)
  ...
  logging.info("grower type %s", grow_type)
  logging.info("grower type %s", grow_type)
  if grower: #si on fait du growing on definit les param associes
    grower.epsilon = config.grow_epsilon #firefky necessite que les poids sortant ne soit pas nul donc il initialise les poids sortant avec une petite valeur=epsilon pour toutes les approches
    grower.scale_method = config.grow_scale_method #je pense qu'il font un scale de la norme des poids pour avoir une comparaison equitable entre approche
  ...
  if config.scale_epochs:# je pense: Scale the trianing epochs to match roughly to big-baseline cost.
    ...
    ############# IMPORTANT: si on n'a pas de gain en energie essayer d'enlever l'ajustement des epochs
  ...
  print('Model number of weights:', model.count_params())
  ...
  updater_type = getattr(config, 'updater_type', None) #il cree l'updater du growing(voir fichier updaters.py)
  ...
  def train_step(iterator, grow_iterator):
    ...
    for ind in tf.range(tf.cast(steps_per_epoch, tf.int32)):
        print("iteration", ind)
        print('device', tf.config.get_visible_devices())
        is_update = updater.is_update_iteration(optimizer.iterations)

        if is_update: #si c'est une iteration ou on fait le growing, on appelle l'updater du growing pour changer la structure du network et faite le growth
            logging.info('Growing on iteration: %s', optimizer.iterations.numpy())
            print("growing on iteration", optimizer.iterations.numpy())
            updater.update_network(batch_data=next(grow_iterator),
                                    optimizer=optimizer)
            compile_model_fn()#on recompile le modele apres le growing
            # We need to remap this as variable names change when the network is grown.
            variable_mapped_grow_metrics = {
                l.weights[0].name: metric for l, metric in grow_metrics.items()
            }
        ...
        print(grads[0].shape)
        shape=grads[0].shape[0] * grads[0].shape[1] * grads[0].shape[2] * grads[0].shape[3]
        clip_val=np.sum(np.mean(grads[0], axis=(0,1,2)))/shape
        print(clip_val)
        import sys
        sys.exit()
  ...
  for epoch in range(initial_epoch, config.train_epochs): #parcours des epochs, boucle de training globale
    ...
    train_step(train_iterator, grow_iterator) #le train_iterator iter sur le train_set et le grow_iterator itere sur le grow_dataset, le batch utilise pour le growing
  ...
  logging.info('Model number of weights at the end of training: %s', model.count_params())
  print('Model number of weights at the end of training:', model.count_params())
