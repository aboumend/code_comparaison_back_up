...
def get_optimizer(optimizer_config, train_epochs, batch_size, steps_per_epoch): #definit l'optimizer avec le lr schedule si necessaire
  ...
  optimizer = tf.keras.optimizers.SGD(lr_schedule,
                                      ...
                                      nesterov=optimizer_config.nesterov, clipvalue=3.165312928440808e-06)
  return optimizer
...
def main(argv):
  ...
  logging.info('training with data selection, selectivity= %s', selectivity)
  ...
  def train_step(iterator):
    ...
    for ind in tf.range(tf.cast(steps_per_epoch, tf.int32)):#parcours des batches/iteration de l'epoch
      ...
      if (ind==0):
        flag_select=False #si c'est l'iter 0 pas de selection, sinon on ne peut pas faire l'optimizer step sur les gradients precedents quand le batch n'est pas selectionne
        logging.info("first iter of training, setting selection flag to false")
  ...
  for epoch in range(initial_epoch, config.train_epochs): #parcours des epochs, boucle de training globale
    ...
    nb_iters, nb_selected, nb_not_selected= train_step(train_iterator) #le train_iterator iter sur le train_set
    ...
    logging.info("nb total batches %s, nb selected batches %s, nb not selected_batches %s", nb_total_iters, nb_selected_batches, nb_not_selected_batches) 
    print("nb total batches", nb_total_iters, "nb selected batches", nb_selected_batches, "nb_not selected batches", nb_not_selected_batches)
