...
import ml_collections #c'est un library ou on a des configdict ou on garde les config des experiments
...
def get_config():
  ...
  config.model.depth = 28 #parce que par defaut le modele est WRN 28-1
  config.model.width_multiplier = 1 #parce que par defaut le modele est WRN 28-1
  config.model.normalization_type = 'none'
  config.model.block_width_multiplier = 0.25 #comme la seed architecture est 25% de l'archi originale
