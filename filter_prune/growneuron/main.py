...
def main(argv):
  ...
  #n_grow=
  n_grow_fraction=0.25
  n_growth_steps=12
  start_iteration=10000

  updater = updaters.AllAtOnce(grower, grow_layer_tuples, loss_fn,
                                   compile_model_fn, update_frequency=update_frequency, n_grow_fraction=n_grow_fraction, 
                                   n_growth_steps=n_growth_steps, start_iteration= start_iteration, carry_optimizer=True)
  ...
  for epoch in range(initial_epoch, config.train_epochs): #parcours des epochs, boucle de training globale
    ...
    train_step(train_iterator) #le train_iterator iter sur le train_set et le grow_iterator itere sur le grow_dataset, le batch utilise pour le growing
