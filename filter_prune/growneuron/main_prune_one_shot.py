...
import updaters_oneshot as updaters
...
def main(argv):
  ...
  #n_grow=
  n_grow_fraction=0.75
  
  prune_epoch=100

  updater = updaters.AllAtOnce(grower, grow_layer_tuples, loss_fn,
                                   compile_model_fn, n_grow_fraction=n_grow_fraction, 
  ...
  def train_step(iterator, is_update):
    ...
    for ind in tf.range(tf.cast(steps_per_epoch, tf.int32)):
        ...
        if (is_update): #si c'est une iteration ou on fait le growing, on appelle l'updater du growing pour changer la structure du network et faite le growth
            logging.info('Model number of weights before pruning: %s', model.count_params())
            print("model number of weights before pruning:", model.count_params())
            logging.info('pruning on iteration: %s', optimizer.iterations.numpy())
            print("pruning on iteration", optimizer.iterations.numpy())
            updater.update_network(batch_data=None,
                                    optimizer=optimizer)
            ...
            logging.info('Model number of weights after pruning: %s', model.count_params())
            print("model number of weights after pruning:", model.count_params())
            """with summary_writer.as_default():
            ...
            is_update=False #on le remets a false sinon ca va chercher a elaguer dans la prochaine iter
  ...
  for epoch in range(initial_epoch, config.train_epochs): #parcours des epochs, boucle de training globale
    ...
    train_step(train_iterator, is_update) #le train_iterator iter sur le train_set et le grow_iterator itere sur le grow_dataset, le batch utilise pour le growing
